(ns jupyter-helpers
  (:require [clojure.string :as string]
            ...
            [ppdsp.dataset.base :refer [get-schema]];;waruni
...
(defn round-known-record-counts
 ...
 ;;Measuring the breach probability after performing attacks on each cycle
(defn attack-per-cycle [results temp-bp-file epsilon attack-count ]
 (def counterX (atom 0))
           (for [result results]
			       	(if  (= (-> result :strategy):a-rpcn) ;;consider the privacy for the attack strategy a-rpcn
                                ...
                                ; x (println "inside let")
				                         count-value (if (<= error epsilon)
						                                    (swap! counterX inc)
								                                (+ @counterX 0) 
                                           ) 
							                       ; bp (for [i (range 1)]
							                                    ;  [(double (/ count-value attack-count))]
							                           ; )
				                         ]
                      ; (double (/ count-value attack-count))
                                           ; (spit temp-bp-file (double (/ count-value attack-count)))
                                           ; (println "error" error)
                                      ; (csv/write-csv writerx bp) 
                                                 ;(.write writerx (double (/ count-value attack-count)))
...
;;Read the results written t0 the file and proccessing privacy results to calculate the breach probability
(defn save-bp-final [ growth-rate-k output-file attack-count cumulative-noise-sigmas epsilon breach-probability-file temp-bp-file ] ;
 (with-open [writer (io/writer breach-probability-file :append true)]
    ...
    ;original-accuracy-cumulative-logistic (-> output-cumulative-logistic :original :accuracy)
           ...
           attacks-cycle (doall(attack-per-cycle flat-results-cumulative-logistic temp-bp-file epsilon attack-count ))
					 final-bp (load-data temp-bp-file)
					          
					 final-breach (for [i (range 1)]
					                            [growth-rate-k final-bp]
					                       )
          ]
   ;(println flat-results-cumulative-logistic)
  ; (println "BP" attacks-cycle )
   (println "Breach probability was calculated")
  ...
  [{:keys [dataset  output-file output-file-privacy-auc output-file-privacy-attacks 
          ...
          attack-count attempt-count known-record-counts known-record-ranges
           known-record-range-position attack-strategies
            ...
            threads-per-configuration  previous-AUC-file-to-update    ;; cycle-size-upper-bound pre-AUC bandwidth
		  ...
		  (time
		   (let [configurations (for [projection-feature-count projection-feature-counts
            ...
            (println "Attacks were performed")
		         (doall (save-bp-final growth-rate-k output-file attack-count cumulative-noise-sigmas epsilon final-bp-file temp-bp-file)) 
