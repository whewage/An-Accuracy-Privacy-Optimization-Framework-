(ns ppdsp.masking.evaluation
  (:require [ppdsp.masking.base :refer [mask-dataset get-masked-schema
            ...
            [ppdsp.dataset.base :refer [get-schema dataset->matrix dataset->classes matrix->dataset
                                       first-record normalise-dataset dataset-record-count]]
...
(defn get-record-bounds
  ...
  (if (= 1 record-range)
    ...
    [0 record-count] ;;flat-record-length
    (case range-position
      :middle (let [onesided-bound (Math/round (* record-count  (* record-range 0.5))) ;;flat-record-length
                    mid-record (quot record-count 2)] ;;flat-record-length
                ;; Return the start and end bounds either side of the middle.
                [(max 0 (- mid-record onesided-bound))
                 (min record-count (+ mid-record onesided-bound))]) ;;flat-record-length
      :start [0 (Math/round (* record-count record-range))])))  ;;flat-record-length
...
(defn generate-io-attack-data!
  "Use the rng to generate data for a known input/output attack (in particular, selecting the known and unknown records).
   Attacks are performed for each cycle" 
  [rng input-matrix masked-matrix projection-sigma cumulative-noise-sigma known-record-count  ;;independent-sigma
   known-record-range known-record-range-position cycle-size cycle-no] ;;  flat-record-length
  (def full-cycle (* cycle-size 2))
   (let [ 
   ;; cycle-no (rand-int 36)  
           cycle-no cycle-no ;(for every cycle) 
           record-count full-cycle            ;;(m/row-count masked-matrix)
           [known-record-start known-record-end] (get-record-bounds record-count known-record-range known-record-range-position)    ;;gives [0 record-count]  ;;flat-record-length 
         ;; Create a derived rng so that the number of known records does not effect subsequent rng in the experiment.
            records-rng (seeded-rng (next-derived-seed! rng))
             unknown-record-index (first (random-li-row-indexes! records-rng input-matrix 1 
                                                               :start-index (- (* record-count cycle-no) known-record-end)
                                                               :end-index ( - (* record-count cycle-no) known-record-start )
                                          )
                                    )
   ...
   ;;  (println "records-rng" records-rng)
    ...
    ;(println "start" (- (* record-count cycle-no) known-record-end))
   ;  (println "end" ( - (* record-count cycle-no) known-record-start ))
...
(defn evaluate-privacy-attacks
  ...
  (def x (atom 0)) ;;waruni
  (def cycles (int (ceil (/(dataset-record-count dataset) (* 2 cycle-size) ))) );needs to change according to the number of records in the data set
					                                 ...
					                                 100
					                                  (generate-io-attack-data! (seeded-rng (next-derived-seed! rng))
					                                                           ...
					                                                           cumulative-noise-sigma
					                                                            ...
					                                                            cycle-size cycle-no) ;;flat-record-length
)
...
(defn calculate-avarage-information-loss [model-description probability-file & [k-value maximum-fn-value]]
  ...
  (with-open [writer (io/writer probability-file :append true)]
		  (let [results (get-probabilities model-description)
            ...
            path-into-leaf (for [res results] ;;path-prob multiplied by leaf-prob
            ...
            AIL (/ path-into-leaf-sum leaf-count)
            prob (if k-value
                        ;;(concat [[ "Growth-Rate-k" "AIL"]]
			                     ...
			                     ;;)
				                    (concat [["Original Average Expected Loss"]]
				                           (for [i (range 1)]
							                            [AIL]
							                      )
                               )
...
(defn getting-original-model-description [dataset model-file loss-file]
  "Getting the model description as moa string and pass it to a clojure data structure.
   This is done because we need to access the elements of the data structure to calculate AEL and it's difficult using moa structure"
       ...
       (calculate-avarage-information-loss (load-data model-file) loss-file)
     )		  
)

(defn getting-masked-model-description [masked-matrix schema classes masked-model-file]
		(let [ model (hoeffding-tree  schema)
          masked-data (matrix->dataset schema masked-matrix classes)
          record (-> masked-data :records)
          record-count (count record)
         ;; dataset-label "arem"
         ;; file (str "workspace/" dataset-label "/masked-model-description.edn")
        ]
    (loop [i 0]
...
(defn masking-experiment-cycles
  [& {:keys [raw-dataset projection-features projection-sigma masked-model-file masked-probability-file
  ...
  ;(for [growth-rate-k growth-rate-k 
   ...
   ;(loop [f bandwidth]
		 ;(when (<= f 0.4)
                 ...
                 model-description (getting-masked-model-description masked-matrix masked-schema masked-classes masked-model-file)
                 masked-model-description (load-data masked-model-file)
                 loss (calculate-avarage-information-loss masked-model-description masked-probability-file growth-rate-k maximum-fn-value) ;f
